...
class FizzBuzzModel(Module):
    def __init__(self, num_hidden: int = 50) -> None:
        self.w1 = Parameter(np.random.randn(10, num_hidden))
        self.b1 = Parameter(np.random.randn(num_hidden))
        self.w2 = Parameter(np.random.randn(num_hidden, 4))
        self.b2 = Parameter(np.random.randn(4))
...
for x in range(1, 101):
    ...
    predicted_idx = np.argmax(predicted.data.asnumpy())
